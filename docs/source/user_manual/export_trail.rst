...
.. code-block::
    ...
    12
    sage: # input_difference = list(cipher.results[0]['assignment']['IN'].values())
    sage: # output_difference = list(cipher.results[0]['assignment']['OUT'].values())
    ...
    sage: # cipher.export(tmpfile_name) ;expects Object 'PRESENT' has been exported to ...
    sage: # Load the cipher from scratch
    sage: # loaded = cipher.load(tmpfile_name)
    sage: # The results are still the same
    sage: # list(loaded.results[0]['assignment']['IN'].values()) == input_difference; expects True
    sage: # list(loaded.results[0]['assignment']['OUT'].values()) == output_difference; expects True
